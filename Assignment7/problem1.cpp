...
int main() {
  ...
  std::vector<double> FD_plot;
  std::vector<double> period;
  std::vector< std::pair<double,double> > poincare_map;
  std::vector< std::pair<double,double> > bifurcation_map;
  double dt_min = tau;
  double dt_max = tau;
  int iStep;
  ...
  for( iStep=0; iStep<nStep; iStep++ ) {  
	...
	bifurcation_map.push_back( std::pair<double,double>(FDplot, omplot*180/pi) );
      }
  ...
  //* Print out the plotting variables: F_plot, omega_plot
  ...
  ofstream bifurcationOut("bifurcation.txt" );
  for (i=0; i< bifurcation_map.size(); i++ ) {
    bifurcationOut << bifurcation_map[i].first << " " << bifurcation_map[i].second << endl;
  }
  bifurcationOut.close();
